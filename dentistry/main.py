...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await safe_reply(update, "✅ CharaNas Dentistry is online. Send /start to open the specialty menu.")
...
def main() -> None:
    token = os.getenv("DENTISTRY_BOT_TOKEN") or os.getenv("BOT_TOKEN")
    if not token:
        raise SystemExit(
            "DENTISTRY_BOT_TOKEN (or BOT_TOKEN) is missing. Copy .env.example to .env and set it."
        )
    ...
    print("CharaNas Dentistry bot running (specialties + difficulty)…")
    app.run_polling(
