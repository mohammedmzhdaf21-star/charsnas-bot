...
log = logging.getLogger("charanas-mls-bot")
...
BOT_TOKEN = os.getenv("MLS_BOT_TOKEN") or os.getenv("BOT_TOKEN")
...
def main() -> None:
    token = os.getenv("MLS_BOT_TOKEN") or os.getenv("BOT_TOKEN")
    if not token:
        raise SystemExit(
            "MLS_BOT_TOKEN (or BOT_TOKEN) is missing. Copy .env.example to .env and set it."
        )
    ...
    log.info("CharaNas MLS bot starting polling…")
    print("CharaNas MLS bot running (specialties + difficulty)…")
    app.run_polling(
