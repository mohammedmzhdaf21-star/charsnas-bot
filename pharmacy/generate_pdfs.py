...
def pdf_filename(specialty_key: str) -> str:
    return f"UG_Pharmacy_{specialty_key.title().replace(' ', '_')}_Notes.pdf"
...
def _header_footer(canvas, doc):
    ...
    canvas.drawString(2 * cm, 1.2 * cm, "CharaNas UG Pharmacy — study aid (not a textbook scan)")
...
def build_story(specialty_key: str) -> list:
    ...
    story.append(Paragraph(f"Undergraduate Pharmacy", styles["subtitle"]))
    story.append(Paragraph(f"{data['label']} Study Pack", styles["cover"]))
    story.append(Paragraph("CharaNas Pharmacy Bot — Specialty Notes", styles["subtitle"]))
    story.append(
        Paragraph(
