...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await safe_reply(update, "✅ Nursing bot is online. Send /start to open the specialty menu.")
...
def main() -> None:
    token = os.getenv("NURSING_BOT_TOKEN") or os.getenv("BOT_TOKEN")
    if not token:
        raise SystemExit(
            "NURSING_BOT_TOKEN (or BOT_TOKEN) is missing. Copy .env.example to .env and set it."
        )
